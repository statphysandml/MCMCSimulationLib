...
namespace mcmc::virtualenv {
    ...
    void virtual_env_integration::set_conda_activate_path(const std::string &conda_activate_path) {
        g_conda_activate_path = conda_activate_path;
        std::cout << " -- Setting conda activate path to '" << g_conda_activate_path << "' --" << std::endl;
    }

    void virtual_env_integration::set_virtual_env(const std::string &virtual_env) {
