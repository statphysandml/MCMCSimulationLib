    ...
    namespace execution {
        ...
        void initialize_python(const std::string python_modules_path_) {
            ...
            if(python_modules_path_ != "")
            {
                ...
                py::exec(("import sys; sys.path.append('" + Executer::get_python_modules_path() + "')").c_str());
            }
        ...
        void Executer::run_execution_on_cpu_cluster(const std::string mode_type) {
        ...
        void Executer::run_execution_on_gpu_cluster(const std::string mode_type) {
            std::cerr << "To be implemented" << std::endl;
            std::exit(EXIT_FAILURE);
        }
