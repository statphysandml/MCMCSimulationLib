...
void ising_model_simulation_with_correlation_time() {
    // Setting up direction parameters
    std::string target_name = "IsingModelSimulationWithCorrelationTime";
    std::string rel_data_path = "/data/" + target_name + "/";
    ...
    // Setting up execution parameters - "measures" can only be defined here if SystemBaseTemplateParameters is used
    typedef mcmc::execution::ExpectationValueParameters ExpectationValueParams;
    ExpectationValueParams execution_parameters(correlation_time_results_path, 10000, 1000, {"AbsMean", "SecondMoment", "Mean", "Config"}, {});
