...
json generate_parameter_class_json(BaseParameterClass& base_parameters, std::string param_class_name) {
    ...
    if (base_parameters.template haskey(param_class_name))
    {
    ...
    else if (base_parameters.template haskey(param_class_name + "_path") and impl_helper::fs::check_if_parameter_file_exists(
            base_parameters.template get_entry<std::string>(param_class_name + "_path"), param_class_name, true)) // -> load json file if possible
    {
        std::string parameter_class_params_path = base_parameters.template get_entry<std::string>(param_class_name + "_path");
        std::cout << "Load params with name " << param_class_name << " from file from " << parameter_class_params_path << std::endl;
        parameter_class_params_ = impl_helper::fs::read_parameter_file(parameter_class_params_path, param_class_name);
        // Verify integrity
